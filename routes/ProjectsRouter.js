...
// get project by id
router.get('/:id', pMid.validateProjectById, (req, res) => {
  const { id } = req.params;
  Projects.get(id)
    .then(resp => {
      res.status(200).json(resp);
...
// update a project
router.put('/:id', pMid.validateProjectById, pMid.validateProject, (req, res) => {
  const { id } = req.params;
...
// delete a project
router.delete('/:id', pMid.validateProjectById, (req, res) => {
  const { id } = req.params;
  Projects.remove(id)
    .then(resp => {
